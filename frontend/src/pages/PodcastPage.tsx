...
export default function PodcastPage() {
  ...
  const handleGenerate = async () => {
    if (!selectedMat && !customText.trim()) {
      return toast.error('Upload a file or enter some text first')
    }
    const result = await dispatch(generatePodcast({
      materialId: selectedMat || undefined,
      style,
    ...
    <div className="p-4 sm:p-6 md:p-8 max-w-3xl mx-auto animate-fade-in">
      ...
      {materials.length > 0 && (
        <div className="mb-4">
          ...
          <select className="input" value={selectedMat} onChange={e => setSelectedMat(e.target.value)}>
            ...
            {materials.map(m => (
              <option key={m._id} value={m._id}>{m.title} ({m.fileType})</option>
            ))}
          </select>
        </div>
      )}
      ...
      <div className="mb-4">
        ...
        <textarea
          className="textarea"
          rows={4}
          placeholder="Paste text you want converted into a podcast lesson…"
          value={customText}
      ...
      {script && (
        <div className="mt-6 card animate-slide-up">
          ...
          <div className="bg-ink-800 rounded-xl p-4 text-sm text-gray-300 leading-relaxed whitespace-pre-wrap max-h-96 overflow-y-auto">
            {script}
          </div>
        </div>
      )}
