    ...
    <div className="p-4 sm:p-6 lg:p-8 max-w-3xl mx-auto animate-fade-in">
      ...
      <div className="card mb-5">
        ...
        <div className="flex gap-2 flex-wrap sm:flex-nowrap">
          <input className="input flex-1 min-w-0" placeholder="e.g. Entschuldigung"
            value={word} onChange={e => setWord(e.target.value)}
            onKeyDown={e => e.key === 'Enter' && handleTeach()}/>
          <button onClick={() => speak(word)} className="btn-secondary px-3 shrink-0"><Volume2 size={16}/></button>
          <button onClick={handleTeach} className="btn-primary shrink-0" disabled={loading}>
            {loading ? <span className="spinner"/> : <Search size={15}/>} Teach Me
          </button>
        </div>
        {lesson && (
      ...
      <div className="card mb-5">
        <h2 className="section-label">German Sounds — tap to hear</h2>
