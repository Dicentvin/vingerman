    ...
    <div className="p-4 sm:p-6 md:p-8 max-w-3xl mx-auto animate-fade-in">
      ...
      <div className="card mb-5">
        ...
        <div className="flex gap-2">
          ...
          />
          <button onClick={() => speak(word)} className="btn-secondary px-3 shrink-0" title="Hear it">
            <Volume2 size={16} />
          </button>
          <button onClick={handleTeach} className="btn-primary shrink-0" disabled={loading}>
      ...
      <div className="card mb-5">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
          {PHONEMES.map(p => (
            ...
            >
              <div className="font-display text-xl sm:text-2xl text-gray-100 group-hover:text-violet-soft">{p.de}</div>
              <div className="text-xs text-violet-soft mt-0.5">{p.ipa}</div>
              <div className="text-xs text-gray-500 leading-tight">{p.en}</div>
            </button>
          ))}
        </div>
      </div>
            ...
            <div key={p.de} className="flex items-center gap-3 py-2.5 border-b border-white/[0.05] last:border-0">
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-gray-500">
                  {p.en}
                  <span className="text-violet-soft ml-1 sm:ml-2">{p.ipa}</span>
                </p>
              </div>
              <button onClick={() => speak(p.de)} className="btn-ghost px-2 shrink-0">
                <Volume2 size={14} />
              </button>
            </div>
          ))}
